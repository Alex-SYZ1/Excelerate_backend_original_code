...
class Excel_attribute:
    ...
    def set_dropdowns(self,selected_field_rules,sep_row=2):
        ...
        dv_dict={}
        def set_dropdowns(one_index_col,one_rule_list,sep_row=sep_row):
            """设置某一字段所在列的下拉列表"""
            
            # 下拉列表dv的相关属性
            formula1_insides_quotes=",".join(one_rule_list)
            dv_col,dv_beginrow=one_index_col[0],int(one_index_col[1:])+sep_row
            sqref=f'{dv_col}{dv_beginrow}:{dv_col}B1048576'
            
            # 添加下拉列表及其对应区域
            dv = DataValidation(type="list", formula1=f'"{formula1_insides_quotes}"', showErrorMessage=False,allow_blank=False)
            self.excel_ws.add_data_validation(dv)
            dv.add(sqref)
            return dv
        for one_index_col in selected_field_rules:
            one_rule_list = selected_field_rules[one_index_col][1]    
            dv_dict[one_index_col[0]]=set_dropdowns(one_index_col,one_rule_list)
        ...
        """def set_dropdowns(one_index_col,one_rule_list,sep_row=sep_row):
            ...
            # 添加下拉列表及其对应区域
            dv = DataValidation(type="list", formula1=f'"{formula1_insides_quotes}"', allow_blank=True)
            self.excel_ws.add_data_validation(dv)
            dv.add(sqref)
        for one_index_col in selected_field_rules:
            one_rule_list = selected_field_rules[one_index_col][1]    
            set_dropdowns(one_index_col,one_rule_list)"""
...
if "__main__" == __name__:
    ...
    xlsx_wb.save("../tests/for_fuker.func2/output1-test_set_dropdown.xlsx")
