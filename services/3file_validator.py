...
class FileValidator:
    ...
    def validate_filled_excel      (self, 
        ...
        for previous_error_cell in self.original_cell_style:
            # print("previous_error_cell",previous_error_cell)
            # print("current_error_cells_info",current_error_cells_info)

            if previous_error_cell not in current_error_cells_info:
                # validated_cell_style =self.original_cell_style[previous_error_cell]
                # delete_or_not.append(previous_error_cell)#(self.original_cell_style.pop(previous_error_cell))
                # self.Xattr.modify_cell_style(previous_error_cell,validated_cell_style)
                # print("validated_cell_style",str(validated_cell_style)[:30])
                
                self.Xattr.modify_cell_style(previous_error_cell,{"font":Font(color='0000FF'),"fill":PatternFill(fill_type=None)})
            else:
                current_error_cells_info[previous_error_cell][-1]=True

        # self.original_cell_style={key:value for key,value in self.original_cell_style.items() if key not in delete_or_not}
        ...
        for current_error_cell,(data_rule,data_example,again_error_flag) in current_error_cells_info.items():
            if 1:#if again_error_flag==False:
...
if __name__ == "__main__":
    ...
    for i in range(1,3):
        ...
        func2_file_name="after_func2_"+str(i)+file_basename+".xlsx"
    ...
    if not cell_info:
        print(f"func3:\n经程序检验，表格无误")
    else:print(cell_info)
    excel_wb_func2=Favor.Xio.load_workbook_from_stream(validated_excel)[0]
    Favor.Xio.save_excel(excel_wb_func2,excel_path=func3_file_save_path)
